...
const ChartsField: React.FC<ChartsBlockProps> = ({
  ...
  const CustomOnchainTxsTooltip = (props) => {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          <p style={{ margin: 0, marginBottom: "6px" }}>{`${formattedDate}`}</p>
          <p style={{ margin: 0, marginBottom: "6px" }}>{`${numberFormatter(
            amount,
          )} coins`}</p>
          <p style={{ margin: 0, marginBottom: "6px" }}>{`$${amountFormatter(
            usdValue,
          )}`}</p>
        </div>
      );
  ...
  const CustomPriceBar = (props) => {
    ...
    let fill =
      chartOption === 1 && date.getDay() === 1
        ? colors.secondary_dark
        : colors.primary_dark;

    const yString = isNaN(y) ? "0" : y.toString();
    return <rect x={x} y={yString} width={width} height={height} fill={fill} />;
  };
  const CustomOnchainVolumeBar = (props) => {
    const { x, y, width, height, date } = props;
    let fill =
      date.getDay() === 1 ? colors.secondary_dark : colors.primary_dark;
    const yString = isNaN(y) ? "0" : y.toString();
    return <rect x={x} y={yString} width={width} height={height} fill={fill} />;
  };
  ...
  return (
    <div style={styles.container} title="Use arrow keys to navigate">
      {availableCharts?.length === 0 && (
        <div style={styles.emptyChartMessage}>Data not (yet) available</div>
      )}
      {availableCharts?.length > 0 && (
        <div style={styles.menuOptions}>
          {Array.from({ length: chartOptionCount }, (_, index) => index).map(
            (option) => (
              <div
                key={option}
      ...
      {(availableCharts[chartOption] === "price30dHistorydata" ||
        availableCharts[chartOption] === "priceMaxHistorydata") && (
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
              ...
              tickFormatter={(date, index) => {
                ...
                if (availableCharts[chartOption] === "priceMaxHistorydata") {
                  ...
                  const interval = Math.ceil(
                    totalDataPoints / desiredTickCount,
                  );
            ...
            <Line
              ...
              dot={false}
            />
            <Bar dataKey="chartFormatVolume" shape={<CustomPriceBar />} />

            <ReferenceLine
              y={
                availableCharts[chartOption] === "price30dHistorydata"
                  ? Math.max(
                      ...price30dHistorydata.map(
                        (dateData) => dateData.chartFormatPrice,
                      ),
                    )
                  : Math.max(
                      ...priceMaxHistorydata.map(
                        (dateData) => dateData.chartFormatPrice,
                      ),
                    )
              }
      ...
      {availableCharts[chartOption] === "txVolumeData" && (
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip
              content={<CustomOnchainVolumeTooltip />}
              cursor={{ fill: "transparent" }}
            />

            <Bar dataKey="volume" shape={<CustomOnchainVolumeBar />} />
            ...
            >
              <Label
                value={`${Math.max(
                  ...txVolumeData.map((dateData) => dateData.volume),
                )}`}
                position="insideBottomLeft"
                fill={colors.secondary_light}
              />
      ...
      {availableCharts[chartOption] === "tokenTxsChartData" && (
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip
              content={<CustomOnchainTxsTooltip />}
              cursor={{ fill: "transparent" }}
            />
            <Bar
              dataKey="usdValue"
            ...
            <ReferenceLine
              y={Math.max(
                ...tokenTxsChartData.map((dateData) => dateData.usdValue),
              )}
              stroke={colors.primary_dark}
              strokeDasharray="0 36 9 0"
            >
            ...
            <ReferenceLine y={20} style={{ display: "none" }}>
              <Label
